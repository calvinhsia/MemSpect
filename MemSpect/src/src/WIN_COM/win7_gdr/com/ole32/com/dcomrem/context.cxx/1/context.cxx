...
#include <ctxconnmgr.hxx>

// includes for WPP tracing
#include <comtrace.h>
#include <context.cxx.tmh>
...
#define MAX_CONTEXT_PROPERTY_SIZE 1024 * 1024
#define MAX_PROPS_ALLOWED 32

static DWORD GetRegSetting(__in LPCWSTR pwszValue, DWORD dwDefault, DWORD* pdwOutput)
{
...
//
//----------------------------------------------------------------------------
__checkReturn STDMETHODIMP CEnumContextProps::Next(
    ULONG            celt,
    __out_ecount(celt) ContextProperty* pCtxProps,
    ...
    )
{
    ...
    ULONG allocSize;
    HRESULT hr = ULongMult(sizeof(IUnknown*), celt, &allocSize);
    if(FAILED(hr))
        return hr;

    IUnknown** PunkArr = (IUnknown**) _alloca(allocSize);
...
BOOL CContextPropList::Grow()
{
    ...
    HRESULT hr = S_OK;
    ULONG cElements = CTX_PROPS;
    ULONG cBytes = 0;
    ULONG cTemp = 0;

    if (_Max)
    {
        hr = ULongMult(_Max, 2, &cElements);
        if (FAILED(hr)) return FALSE;
    }

    ContextProperty* pNewProps = new ContextProperty[cElements];
    if (pNewProps == NULL)
    {
        return FALSE;
    }

    SCtxListIndex* pNewIndex = new SCtxListIndex[cElements];
    if (pNewIndex == NULL)
    {
        delete [] pNewProps;
        return FALSE;
    ...
    int* pNewSlots = new int[cElements];
    if (pNewSlots == NULL)
    {
        delete [] pNewProps;
        delete [] pNewIndex;
    ...
    for (i = (ULONG)_Max; i < cElements; i++)
    {
        pNewIndex[i].idx  = i;
    ...
    // If there are properties, copy the old array into the new array
    if (_Count)
    {
       ...
       do
       {
            pCurProp[propCount] = _pProps[_pIndex[i].idx];
            i = _pIndex[i].next;
       } while((i != _iFirst) && ((++propCount) < cElements));
    ...
    // Update member variables, deleting the existing arrays
    if (_pProps != NULL)
        delete [] _pProps;
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
               "CContextPropList::Add rGUID:%{GUID} flags:%08X pUnk:%p\n",
               &rGUID, flags, pUnk));
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
               "CContextPropList::Get rGUID:%{GUID}\n", &rGUID));
    ...
    )
{
    ...
    if (cProps)
    {
        ...
        hr = E_OUTOFMEMORY;
        pContext = CreateObjectContext(0);
        if (pContext)
        {
...
//+-------------------------------------------------------------------
//
//   Method:   CObjectContext::CreateObjectContext     static
//
// synopsis:   Creates a unique identifier for an object context.
//
//  History:   03-Aug-04  Harsyed   Modified such that it is only used for Direct Context creation
//             11-May-05  aandrejs  Added support for context disconnect 
//+-------------------------------------------------------------------

CObjectContext* CObjectContext::CreateObjectContext (DWORD dwFlags)
{
    ASSERT_LOCK_NOT_HELD(gContextLock);
    ...
    {
        pContext = new CObjectContext(dwFlags, contextId);
    }

    if(pContext)
    {
        //allocation for the context succeeded but not for the support data structurs
        if((dwFlags & CONTEXTFLAGS_DISCONNECTABLE) && !pContext->_pConnectionMgr)
        {
            delete pContext;
            pContext = NULL;
        }
...
//+-------------------------------------------------------------------
//
//   Method:   CObjectContext::CreateEnvoyObjectContext     static
//
// synopsis:   Creates a unique identifier for an object context.
//
//  History:   03-Aug-04  Harsyed   Modified such that it is only used for ONLY EnvoyContext Creation
//
//+-------------------------------------------------------------------

HRESULT CObjectContext::CreateEnvoyObjectContext(
    DATAELEMENT *pDE,
    ...
    )
{
    ASSERT_LOCK_NOT_HELD(gContextLock);
    HRESULT hr = S_OK;
    const DWORD        dwFlags = CONTEXTFLAGS_ENVOYCONTEXT;
    ...
    // process, we can AddRef it and return it.
     ...
     if (pContext == NULL)
     {
        pContext = new CObjectContext(dwFlags, GUID_NULL);
        if (pContext)
        {
            if (FAILED (hr = pContext->SetEnvoyData(pDE)))
            {
                pContext->InternalRelease();
                pContext = NULL;
            }
        ...
        // Note if we fail to allocate the context we will still try to do the lookup here
...
{
    ComDebOut((DEB_OBJECTCONTEXT, "CObjectContext::CObjectContext this:%p\n",this));    
    ...
    RtlZeroMemory ((void*)&_uuidChain, sizeof (_uuidChain));

    if(IsDisconnectable())
    {
        HRESULT hr = S_OK;
        _pConnectionMgr = new CCtxConnectionManager(&hr);
        if(FAILED(hr) || !_pConnectionMgr)
        {
            if(_pConnectionMgr)
            {
                _pConnectionMgr->Release();
                _pConnectionMgr = NULL;
            }

            _dwFlags &= ~CONTEXTFLAGS_DISCONNECTABLE;
        }
    }
    else
        _pConnectionMgr = NULL;
...
CObjectContext::~CObjectContext()
{
    //this has to come first in the destructor
    if(_pConnectionMgr)
    {
        _pConnectionMgr->Release();
        _pConnectionMgr = NULL;
...
{
    //
    // Warning major Perf Hack WINSE 98377
    //
    // There are many places in CLR that call into COM via CoGetObjectContext
    // to determine if they are running in COM+.  Thus this code path is a bit 
    // of a hot spot; commonly, they QI for IObjectContext, so if they are not
    // running in a COM+ context, we end up doing 10 guid comparisons, followed
    // by an optional QI against the aggregated COM+ context if it is there.
    //
    // Optimization:   all of the ten interfaces below end in c000-000000000046.
    // So if the requested interface does not end in that, and we don't have 
    // an aggregated context object, we can reject the QI immediately and save
    // the world tons of wasted CPU time.  Woohoo!  :-)
    //
    // In future if it should become necessary to say yes to an interface that does
    // not end with c000-000000000046 we will have to revisit this choice; maybe a
    // binary search algorithm over a sorted list of valid IIDs?
    //

    BOOL fFoundMatch = FALSE;
    *ppv = NULL;

    if (0xC0 == riid.Data4[0])
    {
    ...
    // If didn't match any of the above guys, check _pMtsContext
    if (!fFoundMatch && _pMtsContext)
    {
...
void CObjectContext::CleanupTables()
{
    ...
    if (IsInPropTable() && GetPropertyList()->GetCount())
    {
        ...
        // The above assert was overactive.  LookupExistingContext might instead
        // find some other context that matches (this is by design).  But we
        // should find some context.
        Win4Assert((pExistingContext != NULL) || IsStatic());
    }
#endif
    ...
    )
{
    ...
    _pifData        =  (InterfaceData *) (((PCHAR) pDE) - sizeof(DWORD)) ;
    _MarshalSizeMax =   pDE->cbSize + SIZEOFDATALEMENT ;
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
     "CObjectContext::GetUnmarshalClass _dwFlags:%08X riid:%{GUID} mshlflags:%08X\n",
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
     "CObjectContext::MarshalInterface _dwFlags:%08X riid:%{GUID} mshlflags:%08X\n",
               _dwFlags, &riid, mshlflags));
    ASSERT_LOCK_NOT_HELD(gContextLock);
...
//      hdr.CmnHdr.Flags        |= CTXMSHLFLAGS_HASMRSHPROP;
        ...
        if (fJustMarshalPointer == TRUE)
        {
            ...
            if (SUCCEEDED(hr))
            {
                // Write out the process guid to protect us against
                // evil imposters.
                GUID guidSecret;
                hr = gProcessSecret.GetProcessSecret(&guidSecret);
                if (SUCCEEDED(hr))
                    hr = pstm->Write(&guidSecret, sizeof(guidSecret), NULL);
                if (SUCCEEDED(hr))
                {
    ...
    )
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
                COMTRACE_LOG2(ERRORS, "Minor:%d %!HRESULT!", 
                    hdr.Version.MinVersion, CO_E_INCOMPATIBLESTREAMVERSION);
                hr = CO_E_INCOMPATIBLESTREAMVERSION;
        }
        else
        {
            // As we dont put any extents we dont expect any, if there are extents we are now 
            // going to error out.
            if ((hdr.CmnHdr.dwNumExtents != 0) || (hdr.CmnHdr.cbExtents != 0))
            {
               COMTRACE_LOG2(ERRORS, "dwNumExtents:%d cbExtents:%d %!HRESULT!", 
                    hdr.CmnHdr.dwNumExtents, hdr.CmnHdr.cbExtents, RPC_E_INVALID_OBJREF);
               return RPC_E_INVALID_OBJREF;
            }
        }
    }
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
               "CObjectContext::UnmarshalInterface _dwFlags:%08X riid:%{GUID}\n",
               _dwFlags, &riid));
    ASSERT_LOCK_NOT_HELD(gContextLock);
    Win4Assert(ppv != NULL);
    ...
    *ppv = NULL;
    ...
    if (MarshaledByReference(hdr))
    {
        ...
        // Check to make sure that the process GUID matches what we
        // expect.  It has to match the GUID we generated when we were
        // initializing the process, or this has been sent by somebody
        // other than us.
        GUID guidCheck;
        hr = pstm->Read(&guidCheck, sizeof(guidCheck), NULL);
        if (hr != S_OK)
            return E_FAIL;
        hr = gProcessSecret.VerifyMatchingSecret(guidCheck);
        if (FAILED(hr))
            return hr;
    ...
    if (Props > MAX_PROPS_ALLOWED)
    {
        hr =  RPC_E_INVALID_OBJREF;      
        COMTRACE_LOG2(ERRORS, "Props:%d %!HRESULT!", 
            Props, hr);
    }
    if (SUCCEEDED(hr) && (Props > 0))
    {
        ...
        do
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                     if  (PropHeader.cb < MAX_CONTEXT_PROPERTY_SIZE)
                     {
                          ulibNextProp.QuadPart += PropHeader.cb;
                          if (ulibNextProp.QuadPart < PropHeader.cb)
                          {
                              hr = RPC_E_INVALID_OBJREF;
                              COMTRACE_LOG3(ERRORS, "%I64u %d %!HRESULT!", 
                                    ulibNextProp.QuadPart, PropHeader.cb, hr);
                          }
                     }
                     else
                     {
                         hr = RPC_E_INVALID_OBJREF;
                         COMTRACE_LOG2(ERRORS, "%d %!HRESULT!", 
                                PropHeader.cb, hr);
                     }
                }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        else
        {
            ...
            // if we are already in the list then we should remove ourselves
            if (NULL != _uuidChain.pNext)
            {
                 // As Fuzzing CoCreates an ObjectContext it is already added in the table
...
{
    ComDebOut((DEB_OBJECTCONTEXT, "CObjectContext::UnmarshalEnvoy riid:%{GUID}\n",
               &riid));
    ASSERT_LOCK_NOT_HELD(gContextLock);
    Win4Assert(ppv != NULL);
    ...
    if (SUCCEEDED(hr) && (Props > 0))
    {
        ...
        do
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr)) ulibNextProp.QuadPart += PropHeader.cb;
            ...
            if (SUCCEEDED(hr))
            {
                ...
                hr  = CoCreateInstance(PropHeader.clsid, NULL, dwFlags,
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    //
                    IUnknown* pUnk = NULL;
                    hr = pME->UnmarshalEnvoy(pstm, IID_IUnknown, (void**) &pUnk);
                    if (SUCCEEDED(hr))
                    {
                        hr = SetProperty(PropHeader.policyId, PropHeader.flags, pUnk);
                        pUnk->Release();
                ...
                {
                    hr = RPC_E_INVALID_OBJREF;
                    COMTRACE_LOG2(ERRORS, "%!GUID! %!HRESULT!", 
                        &PropHeader.clsid, hr);
                }
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
        "CObjectContext::SetProperty GUID:%{GUID} flags:0x08X\n", &rGUID, flags));
    ASSERT_LOCK_NOT_HELD(gContextLock);
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
               "CObjectContext::GetProperty GUID:%{GUID}\n", &rGUID));
    ASSERT_LOCK_NOT_HELD(gContextLock);
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
            "CObjectContext::RemoveProperty GUID:%{GUID}\n", &rGUID));
    ASSERT_LOCK_NOT_HELD(gContextLock);
    ...
    )
{
    ...
    *ppEnumContextProps = NULL;
    ...
    if (!pEnumContextProps)
    {
        delete [] pList;
        pList = NULL;
    }
    else if (!pEnumContextProps->ListRefsOk())
    {
        // in this case pEnumContextProps dtor will cleanup pList
...
STDMETHODIMP CObjectContext::Freeze()
{
    ...
    if (!IsFrozen())
    {
        ...
        if (cnt)
        {
            ...
            ULONG allocSize;
            hr = ULongMult(sizeof(ContextProperty), cnt, &allocSize);
            if(FAILED(hr))
                return hr;

            hr = E_OUTOFMEMORY;
            ContextProperty* pList =
               (ContextProperty *) _alloca(allocSize);
            if (pList != NULL)
            {
                hr = S_OK;
                _properties.SerializeToVector(pList);
                                        ...
                                        unsigned int iMethod)
{
    ...
    ComDebOut((DEB_OBJECTCONTEXT, "CObjectContext::DoCallback [OUT] hr=%#x\n", hr));
    return(hr);
}
                                             ...
                                             IUnknown *pUnk)
{
  ...
  ComDebOut((DEB_OBJECTCONTEXT, "CObjectContext::ContextCallback [OUT] hr=%#x\n", hr));
  return(hr);
}
                                             ...
                                             IUnknown *pUnk)
{
    ...
    ASSERT_LOCK_NOT_HELD(gContextLock);
    ComDebOut((DEB_OBJECTCONTEXT,
               "CObjectContext::InternalContextCallback returning hr:%#x\n", hr));
    return hr;
}
    ...
    if (IsValidPtrOut(pguid, sizeof(*pguid)))
    {
      ...
      if (pguidTmp != NULL)
      {
              *pguid = *pguidTmp;
              ...
              ComDebOut((DEB_OBJECTCONTEXT,
                      "CObjectContext::GetCurrentLogicalThreadId returning hr:S_OK GUID:%{GUID}\n", pguid));
              return S_OK;
      }
      hr = E_OUTOFMEMORY;
    }
    ...
    )
{
    HRESULT hr;
    ComDebOut((DEB_OBJECTCONTEXT,
            "CObjectContext::SetCurrentLogicalThreadId GUID:%{GUID}\n", &rguid));
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
        "CObjectContextCF_CreateInstance pUnkOuter:%p, riid:%{GUID}, ppv:%p\n",
        pUnkOuter, &riid, ppv));
    ASSERT_LOCK_NOT_HELD(gContextLock);
    ...
    HRESULT hr = E_OUTOFMEMORY;
    CObjectContext *pTemp = CObjectContext::CreateObjectContext(0);
...
{
    HRESULT hr = E_OUTOFMEMORY;
    CObjectContext *pCtx = CObjectContext::CreateObjectContext(CONTEXTFLAGS_STATICCONTEXT);
    if (pCtx)
    {
    ...
    )
{
    ...
    if(fMatchApartment)
    {
        ...
        CObjectContext * pCtx1 = CObjectContext::HashPropChainToContext(pNode);
        Win4Assert(pCtx1);
        if (!pCtx1)
            return FALSE;
        CObjectContext * pCtx2 = (CObjectContext*)pv;

        CComApartment * pApt1 = pCtx1->GetComApartment();
        CComApartment * pApt2 = pCtx2->GetComApartment(); 

        if(pApt1 && pApt2)
        {
            if(pApt1->GetAptId() != pApt2->GetAptId()) return FALSE;
        }
        else
        {
            Win4Assert(!"NULL Apartment not expected!");
            if(pApt1 || pApt2) return FALSE;
        }
    ...
    )
{
    ...
    while (pNode != &CCtxTable::s_CtxPropBuckets[iHash])
    {
            ...
            pKey->GetPropertyList()->GetCompareCount() == iCount)
        {
            ...
            if (Compare(pContext, pNode, fMatchApartment))
            {
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
        "CCtxUUIDHashTable::Lookup for riid:%{GUID}\n", &rguid));
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
        "CCtxUUIDHashTable::Add pContext:%p riid:%{GUID}\n",
...
{
    ComDebOut((DEB_OBJECTCONTEXT,
        "CCtxUUIDHashTable::Remove pContext:%p riid:%{GUID}\n",
        pContext, &pContext->GetContextId()));
    ...
    RtlZeroMemory ((void*)pContext->GetUUIDHashChain(), sizeof (SHashChain));
...
//
//----------------------------------------------------------------------------
CObjectContext* CCtxTable::LookupExistingContext(CObjectContext *pContext, BOOL fMatchApartment)
{
    ...
    CObjectContext *pMatchingContext = s_CtxPropHashTable.Lookup(pContext, fMatchApartment);
    if (pMatchingContext)
    {
        // Addref the context
...
INTERNAL PrivGetObjectContext(REFIID riid, void **ppv)
{
    ...
    *ppv = NULL;
    ...
    ContextDebugOut((DEB_OBJECTCONTEXT,
                     "PrivGetObjectContext returning %#x\n", hr));
    return(hr);
}
