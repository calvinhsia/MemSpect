...
{  
    ...
    //      // Create a debug info structure. If this value is set to FALSE, the      // creation of a debug info structure is deferred to the first time      // a thread must block when acquiring the critsec.      //      // This must be done with care to ensure that all relevant parties      // (e.g. the process heap) have access to a critical section debug      // info structure at the appropriate times.      //    
