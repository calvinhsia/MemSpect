...
#define Int3 DebugBreak();
#endif // _X86_
#define VsDebugBreak() do { {Int3; } } while(0) // VsEnsureDebuggerPresent calls GetModuleHandle which can deadlock
...
//usage:
//        VSASSERTF((false, "testing %x  File%s(%d)", 123, __FILE__, __LINE__));

#define VSASSERTF(args)  vsassertf args
...
//
PVOID    VsDebugAllocInternal           (HANDLE hheap, DWORD flags, SIZE_T cb);
PVOID    VsDebugReallocInternal         (HANDLE hheap, PVOID pv, DWORD flags, SIZE_T  cb);
VOID     VsDebugFreeInternal            (HANDLE hheap, PVOID pv);
SIZE_T   VsDebugSizeInternal            (HANDLE hheap, PVOID pv);
HANDLE   VsDebugHeapCreateInternal      (DWORD flags, LPCSTR pszName, LPCSTR pszFile, UINT uLine);
VOID     VsDebugHeapDestroyInternal     (HANDLE hheap, BOOL fLeakCheck);

// debug wrappers for heap allocations.  You should call these to do all of your
// debug memory allocations.
//
#define VsDebAlloc(flags, cb)                 VsDebugAllocInternal(DEFAULT_HEAP, (flags), (cb))
#define VsDebRealloc(pv, flags, cb)           VsDebugReallocInternal(DEFAULT_HEAP, (pv), (flags), (cb))
#define VsDebFree(pv)                         VsDebugFreeInternal(DEFAULT_HEAP, (pv))
#define VsDebSize(pv)                         VsDebugSizeInternal(DEFAULT_HEAP, (pv))

#define VsDebHeapAlloc(heap, flags, cb)       VsDebugAllocInternal(heap, (flags), (cb))
#define VsDebHeapRealloc(heap, pv, flags, cb) VsDebugReallocInternal(heap, (pv), (flags), (cb))
...
#define HeapReAlloc(heap, flags, pv, size)  VsDebugReallocInternal((heap), (flags), (pv), (size))
